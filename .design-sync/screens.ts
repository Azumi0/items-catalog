// The three screen components that live beside their routes under src/app
// rather than in src/components — the item detail view and the login/setup
// forms. Together with src/components they cover every page in the app.
//
...
// are default exports. Naming them here puts all three on
...
// source path, which for a Next.js route means "id", "login" or "setup" —
// route names, not design sections. Going through a directory named for the
// group fixes that at the source instead of fighting it downstream.

// forked from design-sync lib/dts.mjs — this repo's *Manager exports are real screen components, not utility singletons.
//
// The only change is the `Manager` alternative in isComponentName's suffix
// filter, dropped below. CategoriesManager and UsersManager are the category
// and user management screens (src/components/*Manager.tsx) — full React
// components with props, and two of the seven components this repo syncs.
// Upstream's heuristic reads the suffix as a utility-singleton marker, which
// silently dropped both from the emitted component set. No cfg.* key filters
// that list, so the suffix regex is the only place to fix it. `Placements` and
// `Context` are left as upstream has them.
//
...
// *Props are prop interfaces; ALL-CAPS are object constants;
...
//
// FORK: `Manager` removed from the suffix list — see the file header.
